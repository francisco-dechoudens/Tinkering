...
- (id)initWithFrame:(CGRect)frame andBoard:(SHCReversiBoard *)board
{
    if (self = [super initWithFrame:frame])
    {
        ...
        for (int row = 0; row < 8; row++)
        {
            ...
            {
                CGRect squareFrame = CGRectMake(col*columnWidth, row*rowHeight, columnWidth, rowHeight);
                
                SHCBoardSquare* square = [[SHCBoardSquare alloc] initWithFrame:squareFrame column:col row:row board:board];
